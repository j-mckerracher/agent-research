#!/usr/bin/env python3
"""Run the repository's custom Copilot agents in the documented workflow order.

This module turns the rough scratch pseudocode into a deterministic workflow runner
for the custom agents stored in `.github/agents`.
...
STAGE_AGENT_ALIASES: dict[str, tuple[str, ...]] = {
    "orchestrator": ("01-orchestrator", "implementation-orchestrator"),
...
def discover_agents(repo_root: Path) -> dict[str, AgentSpec]:
...
def write_runner_log(config: WorkflowConfig, event_type: str, payload: dict[str, Any]) -> Path:
    """Write a workflow-runner log entry into the orchestrator log folder."""
...
def call_helper_script(config: WorkflowConfig, script_name: str, *args: str) -> CommandResult:
    ...
    script_path = config.repo_root / ".github" / "scripts" / script_name
...
def build_copilot_command(config: WorkflowConfig, agent: AgentSpec, prompt: str) -> list[str]:
    ...
    for extra_dir in config.additional_dirs:
        command.extend(["--add-dir", str(extra_dir)])
    if config.artifact_root != config.repo_root:
        command.extend(["--add-dir", str(config.artifact_root)])
    return command


def build_orchestrator_prompt(config: WorkflowConfig) -> str:
    """Create the non-interactive prompt for the orchestrator agent."""

    return textwrap.dedent(
        f"""
        Automation run: do not ask the user questions.
        ...
        Treat the following context as already supplied by the user and proceed
        directly into workflow intake. Create or refresh the intake artifacts
        under `{config.artifact_root / config.change_id}` and summarize what you
        initialized.
...
def run_workflow(config: WorkflowConfig) -> list[StageResult]:
    ...
    agents = discover_agents(config.repo_root)
    ...
    orchestrator = resolve_agent(agents, "orchestrator")
    invoke_agent(config, orchestrator, build_orchestrator_prompt(config), "orchestrator", 1)
    orchestrator_result = StageResult(
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--repo-root", type=Path, default=Path.cwd(), help="Repository root containing .github/agents")
    ...
    context_group.add_argument("--context", help="Inline workflow context passed to the orchestrator")
    context_group.add_argument("--context-file", type=Path, help="File containing workflow context passed to the orchestrator")
