...
When `intake/story.yaml` indicates a benchmark run (for example `source: phase2-benchmark` or a `benchmark_id` field):

- Evaluate only `planning/assignments.json`, `planning/tasks.yaml`, and local deterministic validator output.
- Do NOT invoke the Reference Librarian, Information Explorer, lessons retrieval flow, or subagents.
- Run the programmatic gates from the repository using repo-relative commands; the benchmark harness already provides the validator scripts.
- Do NOT instruct the workflow to add scripts, commit files, push branches, or perform repository setup work. If a validator fails, report the actual local failure instead.
- Respect the benchmark planner's focused 2-4 task decomposition when checking completeness and dependency ordering.
- A simple benchmark schedule that maps each task to one UoW and places dependent UoWs in later batches is acceptable. Do not fail it just because it omits extra rationale prose or deeper optimization commentary.
- In benchmark mode, warn-only rubric items such as missing rationale text or unquantified parallelism benefit MUST NOT flip `programmatic_gates.all_gates_passed` to false and MUST NOT cause `overall_result: fail` when schema, completeness, dependency order, and duplicate checks all pass.
...
```bash
./.github/scripts/validate-artifact-schema.py --type assignments "$CHANGE_ID/planning/assignments.json"
```

**Dependency cycle detection**:

```bash
./.github/scripts/check-dependency-cycles.py "$CHANGE_ID/planning/assignments.json"
```
...
4. **Pass/Fail Decision Logic**: **PASS** when all critical checks pass and no critical/high issues exist. In benchmark mode, warn-only findings (for example missing batch rationale text) are insufficient for failure.
...
Provide a strict JSON evaluation written to `{CHANGE-ID}/planning/eval_assignments_k.json`. Do not write YAML, comments, frontmatter, or Markdown fences. The file must parse with a standard JSON parser and include these required fields and values:
