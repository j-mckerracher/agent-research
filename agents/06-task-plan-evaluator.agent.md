...
- Evaluate only the provided planning and intake artifacts plus local deterministic validator output.
...
- Do NOT tell the planner to add scripts, commit files, push branches, or perform repository setup work. If a validator fails, report the actual failure from the existing local script output instead.
- Treat a focused 2-4 task benchmark plan as acceptable granularity when it cleanly separates implementation from tests/harness work.
...
```bash
./.github/scripts/validate-artifact-schema.py --type tasks "$CHANGE_ID/planning/tasks.yaml"
```

**Dependency cycle detection**:

```bash
./.github/scripts/check-dependency-cycles.py "$CHANGE_ID/planning/tasks.yaml"
```

**AC coverage completeness**:

```bash
./.github/scripts/check-ac-coverage.py "$CHANGE_ID/intake/story.yaml" "$CHANGE_ID/planning/tasks.yaml"
```
...
- **Granularity (Important)**: Pass for 3-8 broad tasks covering logical phases. For benchmark runs, 2-4 focused implementation-centered tasks are also acceptable. Warn if a non-benchmark plan is too few (<3) or too many (>10); fail if tasks are micro-steps that belong in assignment scheduling.
...
Output must be strict JSON written to `{CHANGE-ID}/planning/eval_tasks_k.json`. Do not write YAML, comments, frontmatter, or Markdown fences. The file must parse with a standard JSON parser and must include:
