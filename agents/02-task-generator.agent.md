...
### Benchmark Intake Shortcut

If `intake/story.yaml` indicates a benchmark run (for example `source: phase2-benchmark` or a `benchmark_id` field) and the intake artifacts already identify the exact target file or directory to change, treat that as sufficient context for task planning.

In that benchmark case:

1. Read the intake artifacts first.
2. Do NOT invoke the Reference Librarian, Information Explorer, or lesson retrieval flow unless a required file location or implementation convention is still genuinely ambiguous after reading the provided inputs.
...
4. If you skip librarian usage, emit `librarian_queries: []` and `librarian_exploration_summaries: []` in `tasks.yaml` and proceed directly to planning.
5. Do NOT create standalone documentation or README tasks unless the benchmark story explicitly requires documentation output.
6. Prefer 2-4 implementation-centered tasks total for benchmark runs. Never emit fewer than 2 tasks because the benchmark evaluator hard-gates task-count >= 2.
7. Use the minimum valid split of (a) implementation work and (b) tests/harness work when a story is small. Additional tasks are only for genuinely separate implementation slices, not for bookkeeping.
8. Do NOT create standalone "verification", "finalization", or "run build/tests" tasks for benchmark runs; workflow hard gates and QA already cover those checks.
9. Only create a distinct test-harness task when harness behavior changes independently of the component-test task; otherwise fold harness updates into the testing task.
