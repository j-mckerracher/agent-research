...
## Phase 2 Benchmark QA Evaluation Mode
...
- Evaluate `qa/qa_report.yaml` against `intake/story.yaml`, `planning/tasks.yaml`, `planning/assignments.json`, every `execution/UOW-*/impl_report.yaml`, and the latest `execution/UOW-*/eval_impl_*.json`.
- Fail QA reports that claim implementation or test evidence is missing when those execution artifacts exist or when passing implementation-evaluator gates already provide that evidence.
- Treat `impl_report.yaml.files_modified`, `definition_of_done_status`, `commands_executed`, and implementation-evaluator `programmatic_gates` as primary traceability evidence for whether QA used the available facts correctly.
- If QA ignores a forced/synthetic-interaction workaround (`click({ force: true })`, `trigger('click')`, `dispatchEvent(new MouseEvent('click'))`, or direct `HTMLElement.click()` dispatch from a Cypress callback) for a newly added visible control, require a non-pass verdict or an explicit condition that a real layout/visibility fix is still needed.
...
- Keep the review single-pass and local.
...
```bash
python3 ./.github/scripts/validate-artifact-schema.py --type qa_report "$CHANGE_ID/qa/qa_report.yaml"
```
