...
name: global-implementation-orchestrator
...
- **Scoped Lesson Delivery**: Before dispatching each stage agent, obtain a bounded `applicable_lessons` set from the Reference Librarian and pass it in invocation context so each agent sees only relevant lessons.
...
| PR / Commit / Branch relations                                            | *(not used — `code_repo` is provided by the user at kickoff)*            | —                            |
...
| QA Agent                 | `global-qa-engineer`              |
| Task Plan Evaluator      | `global-task-plan-evaluator`      |
| Assignment Evaluator     | `global-assignment-evaluator`     |
| Implementation Evaluator | `global-implementation-evaluator` |
| QA Evaluator             | `global-qa-evaluator`             |
...
- **Invoke librarian** (`global-reference-librarian`) — the platform loads its prompt automatically from the `.agent.md` file
...
Run `~/.github/scripts/detect-ui-changes.sh` to determine if UI QA is needed:
```bash
~/.github/scripts/detect-ui-changes.sh $files_modified
# Or: echo "$files_modified" | ~/.github/scripts/detect-ui-changes.sh
```
...
Run `~/.github/scripts/validate-scope.sh` to enforce forbidden file patterns:
```bash
git diff --name-only | ~/.github/scripts/validate-scope.sh --artifact-root "$artifact_root"
```
...
Run `~/.github/scripts/validate-diff-size.sh` to categorize diff size:
```bash
git diff | ~/.github/scripts/validate-diff-size.sh
```
...
Run `~/.github/scripts/init-artifact-dirs.sh` to scaffold the artifact directory tree:
```bash
~/.github/scripts/init-artifact-dirs.sh "$artifact_root" "$CHANGE_ID"
```
