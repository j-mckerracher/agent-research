...
name: global-implementation-evaluator
...
**Schema validation**:
```bash
~/.github/scripts/validate-artifact-schema.py --type impl_report "$CHANGE_ID/execution/$UOW_ID/impl_report.yaml"
```

**Test harness validation**:
```bash
# Extract modified .component.ts files from impl_report, then:
~/.github/scripts/check-test-harnesses.sh $modified_component_files
```
