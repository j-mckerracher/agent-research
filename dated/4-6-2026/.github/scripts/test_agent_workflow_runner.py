#!/usr/bin/env python3
"""Unit tests for `.github/scripts/agent_workflow_runner.py`."""
...
MODULE_PATH = REPO_ROOT / ".github" / "scripts" / "agent_workflow_runner.py"
...
class DiscoverAgentsTests(unittest.TestCase):
	...
	def test_discover_agents_indexes_numbered_and_named_agents(self) -> None:
		agents = module.discover_agents(REPO_ROOT)
		numbered = agents["01-orchestrator"]
		named = agents["implementation-orchestrator"]
		self.assertEqual(numbered.path, named.path)
		self.assertEqual(numbered.name, "implementation-orchestrator")
